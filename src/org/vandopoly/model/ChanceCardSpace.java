...
public class ChanceCardSpace extends Space implements Serializable {
    ...
    protected ChanceCardSpace(ArrayList<Player> players) {
        ...
        players_ = players;
        final Player opponent = players_.get(1);
         ...
         */
        stack_.add(new CardTypePunishPlayers("You may impose a tax on the other player of upto 3500 dollars. " 
                + "The money will be withdrawn from their account, and subtracted from their payment for the experiments. "
                + "Please use the slider to indicate the amount they should be taxed.", 0));
        stack_.add(new CardTypePunishCancel("Player is exempt from tax. "
                + "Any tax they paid will be returned. ", 0));

        itr_ = stack_.listIterator();

        NotificationManager.getInstance().addObserver(Notification.TAX_AI_AMOUNT,
                this, "changeCardAmount");
    }
    ...
    public void landOn(Player p) {
        ...
        if (card.isWithQuestion()) {
            String options[] = {"OK", "Cancel"};
            new MessagePopUp(card, p, players_, "You draw a chance card:", options);
        } else {
            new MessagePopUp(card, p, players_, "The other player draws a chance card:");
        }
    ...
    public void changeCardAmount(Object obj) {
        int amount = Integer.parseInt((String) obj);
        if (stack_.get(1) instanceof CardTypePunishCancel) {
            stack_.get(1).setAmount(amount);
        }
